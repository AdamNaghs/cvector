...
void free_debug_data(Debug_Data d)
{
    ...
    mem_debug_internal_free(d.data);
}
...
void mem_debug_inspect_memory(FILE *stream, uint32_t line, char *file)
{
    ...
    print_char_x_times(stream, '-', DEBUG_MEM_BAR_LENGTH);
    fprintf(stream, "%s\n", MAG);
...
void mem_debug_print_memory(FILE *stream)
{
    size_t i;
    size_t size = vec_size_Vec_Mem(&debug_vec);
    assert(size != SIZE_MAX);
...
void *mem_debug_malloc(size_t size, uint32 line, char *file, char *resize_statement)
{
    ...
    fprintf(stderr,
            BLU "mem_debug_malloc called on line %d, in file %s. Statement used to resize was (%s) is equal to %llu bytes.\n" RESET,
...
#if DEBUG_MEM_PRINT_ALL
        fprintf(stderr,
                YEL "mem_debug_realloc called on line %d, in file %s. Statement used to resize was (%s) is equal to %llu bytes. Pointer named (%s) has been resized %d times.\n" RESET,
...
#if DEBUG_MEM_PRINT_ALL
    fprintf(stderr,
            YEL "mem_debug_calloc called on line %d, in file %s. Statement used to resize was (%s) is equal to %llu bytes.\n" RESET,
            line, file, resize_statement, size);
#endif
    Name n = {.known = false, .name = "Lengthy_Placeholder"};
...
void mem_debug_free(void *ptr, uint32 line, char *file, char *var_name)
{
    ...
    {
        fprintf(stderr,
                RED "mem_debug_free attempting to free memory not allocated with malloc/realloc/calloc (iterated pointer/stack memory/custom malloc) on (line %d, file %s). Variable named (%s)\n" RESET,
                line, file, var_name);
    }
    if (ret.err != VEC_OK)
        return;
...
#if DEBUG_MEM
/* DEBUG MALLOC */
#define malloc(size) (mem_debug_malloc(size, __LINE__, __FILE__, #size))
/* DEBUG REALLOC */
#define realloc(ptr, size) (mem_debug_realloc(ptr, size, __LINE__, __FILE__, #size, #ptr))
/* DEBUG CALLOC */
#define calloc(num, size) (mem_debug_calloc(num, size, __LINE__, __FILE__, #size))
/* DEBUG FREE */
#define free(ptr) (mem_debug_free(ptr, __LINE__, __FILE__, #ptr))
#endif /* DEBUG_MEM */
